...
Banner.prototype.setup = function () {
  this.eCover.center();
  this.bg.set({ right: -60});
  this.bg2.set({ right: '+=120'});
  this.copy.set({ bottom: 18});
  this.copy.centerHorizontal();
  this.title.centerHorizontal();
  this.title.set({ top: 37});
  this.price.centerHorizontal();
  this.price.set({ top: 237});
  this.seats.centerHorizontal();
  this.seats.set({ top: 257, autoAlpha: 0});
  this.cta.centerHorizontal();
  this.cta.set({ bottom: 238});
  this.legal.centerHorizontal();
  this.legal.set({ bottom: 56});
  this.logo.centerHorizontal();
  this.logo.set({ bottom: 11});
...
Banner.prototype.animate = function () {
  ...
  this.timeline = new TimelineLite({paused: false})
    .to([this.eCover, this.bg], 1, {x: '+=120'}, '+=2')
    .from(this.copy, 1, {autoAlpha: 0}, '-=1')
    .to([this.bg, this.eCover], 1.5, {x: '-=240'}, 'yellow+=1')
    .from(this.logo, 1.5, {autoAlpha: 0, x: '+=240'}, 'yellow+=1')
